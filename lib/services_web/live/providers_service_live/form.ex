defmodule ServicesWeb.ProvidersServiceLive.Form do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:form, to_form(ProviderService.change_providers_service(socket.assigns.current_scope, providers_service)))
  end
  ...
  def handle_event("validate", %{"providers_service" => providers_service_params}, socket) do
    changeset = ProviderService.change_providers_service(socket.assigns.current_scope, socket.assigns.providers_service, providers_service_params)
  ...
  defp save_providers_service(socket, :edit, providers_service_params) do
    case ProviderService.update_providers_service(socket.assigns.current_scope, socket.assigns.providers_service, providers_service_params) do
      {:ok, providers_service} ->
        {:noreply,
         socket
         |> put_flash(:info, "Providers service updated successfully")
         |> push_navigate(
           to: return_path(socket.assigns.current_scope, socket.assigns.return_to, providers_service)
         )}
  ...
  defp save_providers_service(socket, :new, providers_service_params) do
    case ProviderService.create_providers_service(socket.assigns.current_scope, providers_service_params) do
      {:ok, providers_service} ->
        {:noreply,
         socket
