defmodule ServicesWeb.UserLive.Edit do
          ...
          <% "provider" -> %>
            <div>
              ...
              <.custom_form for={@user_form} phx-change="validate_user" phx-submit="update_user">
                <.input
                  type="text"
                  placeholder={@provider.users.username}
                  label="Edit username"
                  name="username"
                  value={@provider.users.username}
                />
                <.input
                  type="email"
                  placeholder={@provider.users.email}
                  label="Edit Email"
                  name="email"
            ...
            <div class="m-4">
              ...
              >
                ...
                <.input
                  type="number"
                  placeholder={@provider.years_of_experience}
                  label="years of experience"
                  name="years_of_experience"
                  value={@provider.years_of_experience}
                />
                <.input
                  type="checkbox"
                  placeholder={@provider.is_verified}
                  label="is verified"
                  name="is_verified"
          ...
          <% "providers_service" -> %>
            ...
            <div class="m-4">
              ...
              >
                ...
                />
                <.input
                  field={@provider_form[:years_of_experience]}
                  type="number"
                  placeholder={@provider.years_of_experience}
                  label="years of experience"
                  name="years_of_experience"
            ...
            <%= for service <- @providers_service do %>
              <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-lg transition-all duration-200 overflow-hidden mt-4">
                <.custom_form for={%{}} phx-submit="update_service" phx-change="validate_service">
                  ...
                  <.input
                    ...
                    options={@services}
                    name="service_id"
                    value={service.services.id}
                  />
                  <.input
                    type="number"
                    placeholder={service.custom_price}
                    label="change price"
                    name="custom_price"
                  ...
                  <.input
                    type="checkbox"
                    placeholder={service.is_available}
                    label="is available"
                    name="is_available"
  ...
  def mount(%{"user_id" => user_id}, _session, socket) do
    services =
      Services.Servicing.list_services()
    ...
    socket =
      case Services.ProviderService.get_all_details_of_single_user(user_id) do
        {:ok, %{type: :provider, data: provider}} ->
          ...
          |> assign(
            :provider_form,
            to_form(Services.ServiceProvider.change_provider_admin(provider))
          )
          ...
          |> assign(
            :password_form,
            to_form(
              Services.Accounts.change_user_password(provider.users, %{}, hash_password: false)
            )
          )
        ...
        {:ok, %{type: :user, data: user}} ->
          ...
          |> assign(
            :password_form,
            to_form(Services.Accounts.change_user_password(user, %{}, hash_password: false))
          )
  ...
  def handle_event("validate_provider", provider_params, socket) do
    changeset =
      Services.ServiceProvider.change_provider_admin(socket.assigns.provider, provider_params)
  ...
  def handle_event("update_service", service_params, socket) do
    ...
    case Services.ProviderService.update_providers_service_admin(
           socket.assigns.provider.user_id,
           service,
           service_params
         ) do
