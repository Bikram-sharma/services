    ...
    <div class="h-full p-6 bg-white dark:bg-gray-800  border border-gray-200 dark:border-gray-700 rounded-l-lg space-y-5">
      ...
      <div class="space-y-3">
        <p class="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Service:
          <span class="font-normal text-gray-600 dark:text-gray-400">{@provider_service.services.name}</span>
        </p>
        <p class="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Description:
          <span class="font-normal text-gray-600 dark:text-gray-400">{@provider_service.services.description}</span>
        </p>
        <p class="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Category:
      ...
      <div class="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4 space-y-3">
        ...
        <p class="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Bio:
          <span class="font-normal text-gray-600 dark:text-gray-400">{@provider_service.service_providers.bio}</span>
        </p>
    ...
    <div id="carousel-container" class="overflow-hidden w-full">
      <div id="inner-carousel" class="flex transition-transform duration-900">
        <div class="w-full flex-shrink-0">
          ...
          <!-- ----------Booking Form Start---------- -->
          <.form for={%{}} phx-submit="create_booking_draft" class="h-full dark:text-indigo-400 p-6 bg-white dark:bg-gray-800  border border-gray-200 dark:border-gray-700 rounded-r-lg space-y-6 ">
            <h3 class="text-xl font-bold text-indigo-700 dark:text-indigo-400 border-b border-gray-200 dark:border-gray-700 pb-3 mb-4">
            ...
            <.input required label="contact number" type="number" name="contact" placeholder="provide contact details" value=""/>
            <.input required label="address" name="user_address" placeholder="adress of service" type="text" value=""/>
            ...
            <.input type="hidden" name="actual_total_price" value={@provider_service.custom_price} />
            <div class="flex flex-col sm:flex-row items-center justify-between pt-4 border-t-2 border-dashed border-gray-300 dark:border-gray-600 mt-6">
              ...
              <button type="button" id="next" class="flex items-center text-indigo-700 dark:text-indigo-400">
                Next
              </button>
            </div>
          </.form>
        ...
        <div class="w-full flex-shrink-0">
          ...
          <!-- ---------Service Form Start--------- -->
          <.form for={%{}} phx-submit="book_service" class="p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-r-lg space-y-6">
            <h3 class="text-xl font-bold text-indigo-700 dark:text-indigo-400 border-b border-gray-200 dark:border-gray-700 pb-3 mb-4">
              Enter the service detail
            </h3>

            <.input placeholder="enter brief discussion about the service you are trying to avail." label="description" type="textarea" name="description" value=""/>
            <.input type="datetime-local" label="date of service." name="schedule_date_time" value=""/>
            ...
            <div class="flex flex-col text-gray-700 dark:text-gray-300">
              ...
              <label>
                <input type="checkbox" required class="text-indigo-600 dark:text-indigo-400"> I agree to the Cancellation & Refund Policy <span class="text-red-500">*</span>
              </label>
            </div>

            <div class="flex flex-col sm:flex-row items-center justify-between pt-4 border-t-2 border-dashed border-gray-300 dark:border-gray-600 mt-6">
             <button type="button" id="prev" class="flex items-center cursor-pointer text-indigo-700 dark:text-indigo-400" data-carousel-prev>
                Previous
              </button>
              <button
                type="submit"
...
"""
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    current_time = NaiveDateTime.utc_now()
     ...
     local_time = case DateTime.shift_zone(current_time, "Asia/Thimphu") do
       {:ok, time} ->
        time
        {:error, error} ->
          error
  ...
  case Bookings.create_booking(socket.assigns.current_scope, params) do
    {:ok, booking} ->
      {:noreply,
        ...
        |> put_flash(:info, "booking saved as draft only developer should see this which means the developers should make this button disable after it gets submitted and to another sideof form")}
  ...
  def handle_event("book_service", params, socket) do
    case Services.Bookings.book_service(socket.assigns.current_scope, socket.assigns.booking, params) do
      {:ok, booking} ->
        {:noreply, socket
        |> put_flash(:info, "yeah with this your service has been booked we will get back to you soon")}
      {:error, _changeset} ->
        {:noreply,
        socket
