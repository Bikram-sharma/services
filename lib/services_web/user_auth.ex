defmodule ServicesWeb.UserAuth do
  use ServicesWeb, :verified_routes
  ...
  @remember_me_cookie "_services_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ServicesWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      ServicesWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule ServicesWeb.PageLive do
        ...
        on_mount {ServicesWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ServicesWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Accounts.sudo_mode?(socket.assigns.current_scope.user, -1440) do
      {:cont, socket}
    else
      socket =
        socket
