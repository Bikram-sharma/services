defmodule Services.Accounts do
  @moduledoc """
  ...
  See `Services.Accounts.User.email_changeset/3` for a list of supported options.
...
def list_all_users() do
  ...
  See `Services.Accounts.User.password_changeset/3` for a list of supported options.
...
def delete_user(_, _) do
  ...
  This function sets the `deactivated_at` field of the current user
  (available in `conn.assigns.current_scope.user`) to the current UTC
  timestamp (truncated to seconds) and updates the record in the database.

  It also adds a flash message to the connection:
    - `:info` if the update was successful
