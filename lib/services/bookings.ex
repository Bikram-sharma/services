defmodule Services.Bookings do
  @moduledoc """
  ...
  def subscribe_bookings(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Services.PubSub, "user:#{key}:bookings")
  end

  defp broadcast_booking(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(Services.PubSub, "user:#{key}:bookings", message)
  end
  ...
  def delete_booking(%Scope{} = scope, %Booking{} = booking) do
    ...
    with {:ok, booking = %Booking{}} <-
      ...
      broadcast_booking(scope, {:deleted, booking})
      {:ok, booking}
    end
  end
